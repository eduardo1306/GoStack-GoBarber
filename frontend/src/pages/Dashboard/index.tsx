...
const Dashboard: React.FC = () => {
  ...
  const selectedDateAsText = useMemo(() => {
    const formatedDate = format(selectedDate, "dd 'de' MMMM", {
      locale: ptBR,
    });

    const formatedMonth =
      formatedDate.charAt(6).toUpperCase() + formatedDate.slice(7);

    return format(selectedDate, `dd 'de' '${formatedMonth}'`, {
      locale: ptBR,
    });
  }, [selectedDate]);
  ...
  const selectedWeekDay = useMemo(() => {
    ...
    const formatedWeekDay = weekDay.charAt(0).toUpperCase() + weekDay.slice(1);

    switch (formatedWeekDay) {
      case 'Sabado':
